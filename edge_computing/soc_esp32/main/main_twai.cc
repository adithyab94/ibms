    ...
    while (1)
    {
        ...
        if (ret == ESP_OK)
        {
...

            {
                ...
                memcpy(input->data.f, inputs, sizeof(float) * 300 * 3);
                ...
                {
                    float soc = output->data.f[0];
                    soc  = soc *100;
                    // Add factor 0.01 to soc and scale it
                    
                    ESP_LOGI(TAG, " SOC: %f", soc);

                    uint16_t soc_sc = soc /0.01 ; //soc / 0.01;

                    // Send SOC over CAN
                    twai_message_t soc_msg;
                    soc_msg.identifier = MSG_ID_SOC;
                    soc_msg.extd = 0;
                    soc_msg.rtr = 0;
                    soc_msg.data_length_code = 8;
                    memcpy(soc_msg.data, &soc_sc, sizeof(uint16_t));

                    ret = twai_transmit(&soc_msg, pdMS_TO_TICKS(10));
                    if (ret != ESP_OK)
                    {
                        ESP_LOGE(TAG, "Failed to transmit SOC message: %s", esp_err_to_name(ret));
                    }
